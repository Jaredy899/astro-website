...
Over the past few weeks, I’ve embarked on a new technical challenge: learning the ropes of software packaging for **Solus**. If you’re unfamiliar, Solus is an independently developed Linux distribution known for its performance, stability, and its curated rolling-release model.
...
Every distro has its own way of doing things. In Solus, the magic happens via a tool called `solbuild`. 
...
In some distributions, build recipes can be hundreds of lines of complex scripting. Solus uses a format called **ypkg**, which utilizes a `package.yml` file. 
